...
## 5. Marcas de Agua (Watermarking)

Todo el contenido multimedia (fotos y videos) subido a través de la plataforma debe incluir una marca de agua promocional.
- **Formato de Texto**: `nebulaespace.site/{username}`.
- **Posicionamiento**: Esquina **inferior izquierda** con margen de 20px.
- **Estética**: Texto semi-transparente (blanco con sombra negra sutil) para asegurar legibilidad sobre cualquier fondo.
- **Implementación**:
  - Imágenes: Procesadas con `Pillow`.
  - Videos: Procesados con `ffmpeg` (filtro `drawtext`).
- **Alcance**: Se aplica automáticamente en Posts (Feed) e Historias.

---
*Nota: Este documento debe ser consultado antes de cualquier modificación al sistema de IA o de contenido.*
